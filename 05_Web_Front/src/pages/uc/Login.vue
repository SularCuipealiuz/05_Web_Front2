...
<script>
  ...
  import {getToken, setToken} from "../../config/cookies";
  ...
  export default {
    ...
    methods: {
      ...
      handleSubmit(name) {
        // let result = this._captchaResult;
        const _this = this
        let result = true
        if (!result) {
          $("#notice").show();
          ...
          this.$refs[name].validate(valid => {
            if (valid) {
              ...
              login(params).then(res => {
                _this.$Message.success(_this.$t("uc.login.success"));
                setToken('token', res.data.token)
                setToken('member', _this.formInline.user)

                _this.$store.commit("SET_MEMBER", _this.formInline.user);
                console.log(getToken('token'))
                _this.$router.push("/uc/safe");
              }).catch(err => {
