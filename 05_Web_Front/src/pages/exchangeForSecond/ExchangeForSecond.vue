<template>
  <div class="container exchange" :class="skin">
      ...
      <div class="right">
        <div class="coin-menu">
          <div style="padding: 8px 10px;height:48px;">
            <Input search :placeholder="$t('common.searchplaceholder')" @on-change="searchInputChange"
                   v-model="searchKey"/>
          </div>
          <div class="sc_filter">
            <span v-show="isLogin" @click="changeBaseCoin('favor')" :class="{active:basecoin==='favor'}">{{$t('service.CUSTOM')}}</span>
            <span @click="changeBaseCoin('usdt')" :class="{active:basecoin==='usdt'}">USDT</span>
            <span @click="changeBaseCoin('btc')" :class="{active:basecoin==='btc'}">BTC</span>
            <span @click="changeBaseCoin('eth')" :class="{active:basecoin==='eth'}">ETH</span>
          </div>
          <Table @on-current-change="gohref" highlight-row id="USDT" v-show="basecoin==='usdt'" :columns="coins.columns"
                 :data="dataIndex"></Table>
          <Table @on-current-change="gohref" highlight-row id="BTC" v-show="basecoin==='btc'" :columns="coins.columns"
                 :data="dataIndex"></Table>
          <Table @on-current-change="gohref" highlight-row id="ETH" v-show="basecoin==='eth'" :columns="coins.columns"
                 :data="dataIndex"></Table>
          <Table @on-current-change="gohref" highlight-row v-show="basecoin==='favor'"
                 :no-data-text="$t('common.nodata')" id="collect" :columns="favorColumns" :data="dataIndex"></Table>
          <!--
                    <p v-show="basecoin!='favor'" style="height:40px;line-height:40px;padding-left:10px;border-bottom:1px solid #26303d;font-size:14px;color:rgb(97, 119, 146);">创新版</p>
                    <Table @on-current-change="gohref" highlight-row id="USDT2" v-show="basecoin==='usdt'" :columns="coins.columns" :data="dataIndex2"></Table>
                    <Table @on-current-change="gohref" highlight-row id="BTC2" v-show="basecoin==='btc'" :columns="coins.columns" :data="dataIndex2"></Table>
                    <Table @on-current-change="gohref" highlight-row id="ETH2" v-show="basecoin==='eth'" :columns="coins.columns" :data="dataIndex2"></Table>
          -->
        </div>
      </div>
      <div class="center">
          ...
          <div class="item" style="margin-left: -40px;">
            ...
            <Poptip trigger="hover" :title="coinInfo.name" content="content" placement="bottom-start" word-wrap
              ...
              <div class="api" slot="content">
                ...
                <p style="text-align:right;margin-top: -10px;"><a :href="coinInfo.infolink" target="_blank">{{$t("exchange.moredetail")}}</a>
                </p>
              </div>
            </Poptip>
          </div>
        ...
        <div class="trade_wrap">
          <div class="trade_panel trade_panel_logout">
            ...
            <div class="trade_menu">
              <span @click="toggleCommodityTab" :class="{active:commodityTab}">{{$t("exchange.commodity_tab")}}</span>
            ...
            <div class="trade_bd">
              <div class="panel">
                ...
                <div class="commodity-panel">
                  <div class="commodity-item" :class="{'active': selectedChooseItem === e}"
                       v-for="(e, i) in payChooseList" @click="selectedChooseItem = e">
                    <span>{{e}}</span>
                  </div>
                </div>
                <div class="bd bd_commodity" v-show="commodityTab">
                  <Form ref="formValidate">
                    <Button v-if="exchangeable == 1" class="bg-green" @click="buyUpOrDown('up')">
                      {{$t("exchange.buyups")}}
                    </Button>
                    <Button v-else class="bg-gray">{{$t("exchange.buyups")}}</Button>
                  </Form>
                </div>
              </div>
              <div class="panel">
                <div>开仓时间</div>
                <div class="commodity-panel">
                  <div class="commodity-item" :class="{'active': selectedScaleItem.proTime === e.proTime}"
                       v-for="(e, i) in timeScaleList" @click="selectedScaleItem = e">
                    <span>{{e.proTime}}s</span>
                  </div>
                  <div class="operating-panel">
                    <span class="label">营利率</span>
                    <span v-if="selectedScaleItem.proScale !== ''"
                          class="operating">{{selectedScaleItem.proScale}}%</span>
                  </div>
                </div>
                <div class="bd bd_commodity" v-show="commodityTab">
                  <Form ref="formValidate">
                    <Button v-if="exchangeable == 1" class="bg-red" @click="buyUpOrDown('down')">
                      {{$t("exchange.buydowns")}}
                    </Button>
                    <Button v-else class="bg-gray">{{$t("exchange.buydowns")}}</Button>
                  </Form>
                </div>
              </div>
            </div>
    ...
    <div class="order">
      ...
      <div class="table">
        <Table height="240" v-if="selectedOrder==='current'" :columns="currentOrder.columns" :data="currentOrder.rows"
...
<style scoped lang="scss">
  ...
  .commodity-panel {
    ...
    > .commodity-item {
      ...
      align-items: center;
      background-image: linear-gradient(90deg, rgb(41, 138, 151), rgb(49, 118, 153));
      border-radius: 2px;
...
<script>
  import {tradeInfo} from "@/api/index"
  ...
  export default {
      ...
      return {
        ...
        plate: {
          ...
          columns: [
            ...
            //     const title =
            //       (row.direction == "BUY"
            //         ? self.$t("exchange.buyups")
            //         : self.$t("exchange.buydowns")) +
            //       " " +
            //       row.position;
            //     return h(
            //       "span",
          ...
          columns: [
            ...
            },
            {
              ...
              render(h, params) {
                return h(
                  "span",
                  self.toFloor(params.row.buyprice)
                );
              }
            },
            {
            ...
            },
            {
              ...
              key: "price",
              render(h, params) {
                return h("span", self.toFloor(params.row.buyprice * (params.row.endloss / 100)));
              }
            },
            {
              title: "倒计时",
              ...
              render(h, params) {
                ...
                const row = params.row;
                const sellTime = row.selltime
                let second = 0

                console.log(sellTime, timeStamp)

                if (timeStamp < sellTime) {
                  second = sellTime - timeStamp
                } else {
                  second = '計算中'
                }
                return h(
    ...
    filters: {
      ...
      toFixedPublishPrice: function (value) {
        let tem = value.toFixed(5).replace(/0+$/g, "");
        if (value >= 1) {
          tem = value.toFixed(4);
        }
        if (value >= 10) {
    ...
    methods: {
      ...
      init() {
        ...
        if (params == undefined) {
          this.$router.push("/exchangeForSecond/" + this.defaultPath);
          params = this.defaultPath;
        }
        const basecoin = params.split("_")[1];
        if (basecoin) {
        ...
        this.$store.commit("NAVIGATE", "nav-exchange-for-second");
        this.$store.commit("SET_SKIN", this.skin);
        // this.getCNYRate();
        // this.getSymbolScale();
        // this.getCoinInfo();
        this.getSymbol(); //包含 K线图、getFavor、startWebSocket等
        // this.getPlate(); //买卖盘
        // this.getPlateFull();
        this.getTrade();
        if (this.isLogin) {
      ...
      getSymbol() {
        ...
        // });
        this.startWebSocket();
        this.changeBaseCoin(this.basecoin);
      },
      getCoinInfo() {
        //获取精度
        this.$http
        ...
        this.$http
          ...
          .then(response => {
            ...
            if (resp != null) {
              ...
              let temCurT = moment(resp.currentTime).format("YYYY-MM-DD HH:mm:ss");
      ...
      getTrade() {
        const _this = this
        tradeInfo("11").then(res => {
          _this.payChooseList = res.data.payChoose
          _this.timeScaleList = res.data.timeScale
        })
        // let params = {};
        // params["symbol"] = this.currentCoin.symbol;
        // params["size"] = 20;
      ...
      stopKlineWs() {
        this.webSocket.send(JSON.stringify({
          resource: "KlineNow",
          action: "StopAllKlineNow",
          data: {}
        }))
      },
      toggleCommodityTab() {
        this.commodityTab = true;
      },
      ...
      buyUpOrDown(str) {
        if (this.selectedChooseItem === null) {
          this.$Notice.error({
            title: this.$t("exchange.tip"),
            desc: this.$t("exchange.paychoosenotnull")
          });
          return;
        }
        if (this.selectedScaleItem.proTime === "") {
          this.$Notice.error({
            title: this.$t("exchange.tip"),
            desc: this.$t("exchange.timescalenotnull")
          });
          return;
        }
        let _this = this;
        const payload = JSON.stringify({
          ...
          data: {
            ...
            uid: 106,
            order_type: str === 'up' ? 0 : 1,
            order_pid: 11,
            order_price: this.selectedChooseItem,
            order_sen: this.selectedScaleItem.proTime
          }
        })
      ...
      getCurrentOrder() {
        ...
        params["pageSize"] = 100;
        params["symbol"] = this.currentCoin.symbol;
        this.$bus.$on("updateCurrentDelegate", function (res) {
          console.log('進了!!!', res)
          _this.currentOrder.rows.push(res)
        })
      },
